...
describe Game do
  ...
  describe '#play' do
    before do
      ...
      allow(game).to_receive(:gets).and_return(single_player_mode)
      allow(game).to_receive(:game_loop).once
    end
    ...
    xit 'starts game_loop' do
      expect(game).to_receive(:game_loop).once
      game.play
    end
  end
  ...
  describe '#select_mode' do
    ...
    before do
      allow(game).to_receive(:print).with('Input 1 for single player or 2 for multiplayer: ').once
      allow(game).to_receive(:gets).and_return(mode_selection)
    end

    xit 'prompts player for mode' do
      expect(game).to_receive(:print).with('Input 1 for single player or 2 for multiplayer: ').once
      game.select_mode
  ...
  describe '#verify_mode' do
    ...
    context 'when given invalid input' do
      before do
        valid_input = 1
        allow(game).to_receive(:puts).with('Invalid input!').once
        allow(game).to_receive(:select_mode).once
        allow(game).to_receive(:gets).and_return(valid_input)
      end

      xit 'prompts player for new input' do
        invalid_input = 'b'
        expect(game).to_receive(:puts).with('Invalid input!').once
        expect(game).to_receive(:select_mode).once
        expect(game).to_receive(:gets).and_return(valid_input)
  ...
  describe '#game_loop' do
    context 'when game_over? is true' do
      before do
        # set board
        allow(game).to_receive(:end_game)
      end

      xit 'ends game' do
        expect(game).to_receive(:end_game)
        game.game_loop
      end
    end
  ...
  describe '#select_move' do
    xit 'prompts player for move' do
      valid_input = 4
      allow(game).to_receive(:gets).and_return(valid_input)
      expect(game).to_receive(:print).with('Input the culumn you would like to drop your marker 1-7: ')
      expect(game).to_receive(:gets).and_return(valid_input)
      expect(game).to_receive(:verify_move)
      game.select_move
    end
  end
  ...
  describe '#verify_move' do
    context 'when given valid input' do
      xit 'returns input' do
        valid_input = 4
        expect(game.verify_move(valid_input)).to_return(valid_input)
        game.verify_move
      end
    end
    ...
    context 'when given invalid input' do
      xit 'prompts player for new input' do
        ...
        invalid_input = 'b'
        expect(game).to_receive(:puts).with('Invalid input!').once
        expect(game).to_receive(:select_move).once
        expect(game).to_receive(:gets).and_return(valid_input)
  ...
  describe '#end_game' do
    ...
    context 'player wins' do
      ...
      xit 'declares player winner' do
        expect(game).to_receive(:puts).with("'#{game.player_marker}' wins!")
      end
    end
    ...
    context 'second_player wins' do
      ...
      xit 'declares player winner' do
        expect(game).to_receive(:puts).with("'#{game.second_marker}' wins!")
      end
    end
  ...
  describe '#prompt_new_game' do
    before do
      valid_input = 'n'
      allow(game).to_receive(:print).with('Would you like to play again? y/n: ')
      allow(game).to_receive(:gets).and_return(valid_input)
      allow(game).to_receive(:verify_new_game_input).and_return(valid_input)
    end

    xit 'prompts player for new game' do
      expect(game).to_receive(:print).with('Would you like to play again? y/n: ')
      expect(game).to_receive(:gets).and_return(valid_input)
      expect(game).to_receive(:verify_new_game_input).and_return(valid_input)
      game.select_move
    end
  end
  ...
  describe '#verify_new_game_input' do
    ...
    context 'when given invalid input' do
      before do
        allow(game).to_receive(:puts).with('Invalid input!').once
        allow(game).to_receive(:prompt_new_game).once
      end

      xit 'prompts player for new input' do
        valid_input = 'b'
        expect(game).to_receive(:puts).with('Invalid input!').once
        expect(game).to_receive(:prompt_new_game).once
        game.verify_new_game_input(valid_input)
      end
    end
  end
